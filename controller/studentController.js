...
module.exports = {
  ...
  getPrivateChat: async (req, res, next) => {
    try {
      ...
      const allMessage = await Message.find({ roomId })
        ...
        .populate({ path: "receiverId", select: "avatar" })
  ...
  getAllSubjects: async (req, res, next) => {
    try {
      ...
      const subject = await Subject.find({ _id: { $in: subjects } })
      console.log(subject)
      if (subject.length === 0) {
        return res.status(404).json({ message: "No subject founds" })
      }
  ...
  getFiles: async (req, res) => {
    const result = await PdfFile.find({})
    if (!result) return res.status(404).json({ message: "No any uploads" })
    res.status(200).json({ message: "fetched ", result })
  },
  studentGetCourses: async (req, res) => {
    try {
  ...
  studentGetVideos: async (req, res) => {
