...
module.exports = {
    ...
    addStudent: async (req, res, next) => {
        try {
            ...
            const { name, email, year, fatherName, cnic,
    ...
    uploadFile:async(req,res)=>{
        ...
        const {department,subjectName}=req.body
        let filename =  await bufferConversion(req.file.originalname, req.file.buffer)
        // return console.log(fileName)
        if(req.file.mimetype!=='application/pdf')
        return res.json({message:"Please select a pdf file"})

         if(!filename||!department||!subjectName){
        ...
        // const file=await PdfFile.find()
        const newPdfFile=await new PdfFile({filename,department,subjectName})
